    ...
    function recipe_archive_shortcode($att){
        ...
        <div class="home-content">
            <div class="recipe-card-parent-container" style="width: 100%;">
                <?php 
                    $recipe_card_cont_class =  $values['type'] == 'slider'? 'slider': 'recipe-card-container';
                    $recipe_card_cont_style = $values['type'] == 'slider'? '': 'grid-template-columns:  repeat(' . esc_attr($values['num-columns']) . ', 1fr);';
                
                ?>
                <div class="<?php $recipe_card_cont_class ?>" style="<?php $recipe_card_cont_style ?>"> 
                    <!--  recipe-card-container its main perpose is to add grid but for slide we dont need it so we catch replace with slider-->
                    <!-- all recipe card goes here using js by collecting data from database -->
                    <?php
